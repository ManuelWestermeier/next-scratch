...
const KEY_OPTIONS = ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Space', 'Any'] as const

function isContainerBlock(type: string) {
...
export function BlockView({ block, depth, onDelete, onUpdate, scriptId, objectId, allBlocks, onBlocksChange }: Props) {
  ...
  const isContainer = def.hasChildren || def.isContainer || isContainerBlock(block.type)
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          <div className="flex items-center gap-1 flex-wrap flex-1 min-w-0">
            {block.params.map(param => {
              const currentValue = String(param.value ?? param.defaultValue ?? '')
              return (
                <div key={param.id} className="flex items-center gap-1">
                  {block.params.length > 1 && <span className="text-[10px] opacity-40">{param.name}:</span>}
                  {param.name === 'op' ? (
                    <select
                    ...
                    <input
                      ...
                      style={{ width: Math.max(36, currentValue.length * 7 + 16) }}
                      value={currentValue}
                      onChange={e => handleParamChange(param.id, e.target.value)}
        ...
        {isContainer && !collapsed && (
          <div className="ml-4 mb-1 mr-1">
            {def.hasChildren && (
              <div className="mb-1">
                {def.hasElse && <div className="text-[9px] uppercase tracking-wider opacity-40 px-1 mb-0.5" style={{ color: def.color }}>dann</div>}
                <div
                  className={`min-h-6 rounded border transition-colors ${
                    dropzone === 'children' ? 'border-white/30 bg-white/5' : 'border-dashed border-white/10'
                  }`}
            ...
            {def.hasElse && (
              <div className="mb-1">
                ...
                <div
                  className={`min-h-6 rounded border transition-colors ${
                    dropzone === 'else' ? 'border-white/30 bg-white/5' : 'border-dashed border-white/10'
                  }`}
